...
export class UsersService {
  ...
  async findAll(): Promise<User[]> {
    const users = await this.db.queryMany<User>('SELECT * FROM users ORDER BY created_at DESC');
    return users;
  }
  ...
  async updateRole(id: string, role: string): Promise<User> {
    const user = await this.db.queryOne<User>('SELECT id FROM users WHERE id = $1', [id]);
